...
private:
	...
	void updateNeighbour(Node v, Node w) {
		...
		if (g->id(v) != g->id(w)) {
			...
			prior.set(w,190*ed[w] + 120*dn[w] + sp[w]);
		}
	}
...
public:
	...
	state(*chdata.graph, -1), prior(state), sp(*chdata.graph ,0), dn(*chdata.graph, 0), ed(*chdata.graph, 0),
	...
	Node nextNode() {
		...
		while (!prior.empty()) {
			v = prior.top();
			edgediff(v);
			prior.set(v, 190*ed[v] + 120*dn[v] + sp[v]);
			if (v == prior.top()) {
				prior.pop();
				return v;
			}
